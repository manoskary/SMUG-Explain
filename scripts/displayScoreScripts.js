...
verovio.module.onRuntimeInitialized = async (_) => {
  ...
  const feature_importance_directory = document.getElementById(
  ...
  /**
    ...
    */
  document
    .getElementById("playMIDI")
    .addEventListener("click", playMIDIHandler);
  document
    .getElementById("stopMIDI")
    .addEventListener("click", stopMIDIHandler);
  /**
  ...
  jsonFileInput.addEventListener("change", (event) => {
    ...
    reader.addEventListener("load", (event) => {
      try {
        ...
        instructions.textContent = "Invalid JSON file.";
        return;
      }
      ...
      if (meiFile !== null) {
        ...
        instructions.textContent =
          "Click on notes corresponding to cadences to display explanations.";
      } else {
        instructions.textContent = "Upload MEI score";
        meifileInput.click(); // Optional: auto-trigger MEI upload
      }
    });
  ...
  meifileInput.addEventListener("change", (event) => {
    ...
    if (jsonData === null) {
      ...
      displayScoreWithGraph(meiFile, jsonData, tk);
      instructions.textContent =
        "Use toggle above to move between Input graph and explanations.";
    }
  });
  ...
  document
    .getElementById("inputGraphButton")
    .addEventListener("click", function () {
      ...
      const explanationEdges = document.querySelectorAll(
        "[class$=_explanation_edge]"
      );
      ...
      instructions.innerHTML =
        '<span style="color: red;">Red:</span> consecutive, <span style="color: blue;">Blue:</span> onset, <span style="color: green;">Green:</span> during, <span style="color: yellow;">Yellow:</span> rest';
  ...
  document
    .getElementById("explanationsButton")
    .addEventListener("click", function () {
      this.classList.add("active");
  ...
  reader.onload = (event) => {
    ...
    notes.forEach((note) => {
      note.addEventListener("click", (event) => {
        ...
        const otherEdges = document.querySelectorAll(
          `[class$=_edge]:not(.${note.id}_explanation_edge)`
        );
...
function addEdges(edgeType, jsonGraphAnnotation, pageElement, zip, color) {
  ...
  for (const [start, end] of zip(
    jsonGraphAnnotation[edgeType][0],
    jsonGraphAnnotation[edgeType][1]
  )) {
    const element1 = pageElement.querySelector(
      `#${jsonGraphAnnotation.id[start]} use`
    );
    const element2 = pageElement.querySelector(
      `#${jsonGraphAnnotation.id[end]} use`
    );
    ...
    if ([x1, y1, x2, y2].some((v) => isNaN(v))) {
      console.warn(`Invalid coordinates for edge: ${start} -> ${end}`, {
        x1,
        y1,
        x2,
        y2,
    ...
    console.log(`Edge created: ${start} -> ${end}`, {
      x1,
      y1,
      x2,
      y2,
...
function addExplanations(jsonGraphAnnotation, pageElement, zip, color) {
  ...
  for (const note_idx in note_ids) {
    ...
    const all_src_edges = onset_edges[0].concat(
      during_edges[0],
      rest_edges[0],
      consecutive_edges[0]
    );
    const all_dest_edges = onset_edges[1].concat(
      during_edges[1],
      rest_edges[1],
      consecutive_edges[1]
    );
    for (const [start, end] of zip(all_src_edges, all_dest_edges)) {
      ...
      const src_note = pageElement.querySelector(
        `#${jsonGraphAnnotation.id[start]} use`
      );
      const dest_note = pageElement.querySelector(
        `#${jsonGraphAnnotation.id[end]} use`
      );
